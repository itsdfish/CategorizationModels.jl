...
@safetestset "make_intensity_matrix" begin
    @safetestset "BayesianModel" begin
        ...
        v = make_intensity_matrix(model, 5, 0.5)
    ...
    @safetestset "Hamiltonian" begin
        ...
        x_true = [
            0.16667 0.50000 0.00000 0.00000 0.00000 0.00000 0.00000 0.00000 0.00000 0.00000 0.00000 0.00000;
        ...
        x = make_hamiltonian(model, n_states, 2, 0.5)
...
@safetestset "logpdf" begin
    ...
    @safetestset "BayesianModel" begin
        ...
        υ_ks_ks = range(υ_ks_k * 0.8, υ_ks_k * 1.2, length = 100)
        LLs = map(
            υ_ks_k -> sumlogpdf(BayesianModel(; parms..., υ_ks_k), n_options, data),
            υ_ks_ks
        )
        _, idx = findmax(LLs)
        @test υ_ks_ks[idx] ≈ υ_ks_k rtol = 0.05

        υ_sk_ss = range(υ_sk_s * 0.8, υ_sk_s * 1.2, length = 100)
        LLs = map(
            υ_sk_s -> sumlogpdf(BayesianModel(; parms..., υ_sk_s), n_options, data),
            υ_sk_ss
        )
        _, idx = findmax(LLs)
        @test υ_sk_ss[idx] ≈ υ_sk_s rtol = 0.05

        λ_ks_ks = range(λ_ks_k * 0.8, λ_ks_k * 1.2, length = 100)
        LLs = map(
            λ_ks_k -> sumlogpdf(BayesianModel(; parms..., λ_ks_k), n_options, data),
            λ_ks_ks
        )
        _, idx = findmax(LLs)
        @test λ_ks_ks[idx] ≈ λ_ks_k rtol = 0.05

        λ_sk_ss = range(λ_sk_s * 0.8, λ_sk_s * 1.2, length = 100)
        LLs = map(
            λ_sk_s -> sumlogpdf(BayesianModel(; parms..., λ_sk_s), n_options, data),
            λ_sk_ss
        )
        _, idx = findmax(LLs)
        @test λ_sk_ss[idx] ≈ λ_sk_s rtol = 0.05
    end
    ...
    @safetestset "Markov" begin
        ...
        data = rand(model, preds, n_trials)
        μs = range(μ * 0.8, μ * 1.2, length = 100)
        LLs = map(μ -> sumlogpdf(MarkovModel(; parms..., μ), n_options, data), μs)
        _, idx = findmax(LLs)
        @test μs[idx] ≈ μ rtol = 0.10

        σs = range(σ * 0.8, σ * 1.2, length = 100)
        LLs = map(σ -> sumlogpdf(MarkovModel(; parms..., σ), n_options, data), σs)
        _, idx = findmax(LLs)
        @test σs[idx] ≈ σ rtol = 0.10

        υ_k_ks = range(υ_k_k * 0.8, υ_k_k * 1.2, length = 100)
        LLs =
            map(υ_k_k -> sumlogpdf(MarkovModel(; parms..., υ_k_k), n_options, data), υ_k_ks)
        _, idx = findmax(LLs)
        @test υ_k_ks[idx] ≈ υ_k_k rtol = 0.10

        υ_s_ks = range(υ_s_k * 0.8, υ_s_k * 1.2, length = 100)
        LLs =
            map(υ_s_k -> sumlogpdf(MarkovModel(; parms..., υ_s_k), n_options, data), υ_s_ks)
        _, idx = findmax(LLs)
        @test υ_s_ks[idx] ≈ υ_s_k rtol = 0.10

        υ_k_ss = range(υ_k_s * 0.8, υ_k_s * 1.2, length = 100)
        LLs =
            map(υ_k_s -> sumlogpdf(MarkovModel(; parms..., υ_k_s), n_options, data), υ_k_ss)
        _, idx = findmax(LLs)
        @test υ_k_ss[idx] ≈ υ_k_s rtol = 0.10

        υ_s_ss = range(υ_s_s * 0.8, υ_s_s * 1.2, length = 100)
        LLs =
            map(υ_s_s -> sumlogpdf(MarkovModel(; parms..., υ_s_s), n_options, data), υ_s_ss)
        _, idx = findmax(LLs)
        @test υ_s_ss[idx] ≈ υ_s_s rtol = 0.10

        λ_k_ks = range(λ_k_k * 0.8, λ_k_k * 1.2, length = 100)
        LLs =
            map(λ_k_k -> sumlogpdf(MarkovModel(; parms..., λ_k_k), n_options, data), λ_k_ks)
        _, idx = findmax(LLs)
        @test λ_k_ks[idx] ≈ λ_k_k rtol = 0.10

        λ_s_ks = range(λ_s_k * 0.8, λ_s_k * 1.2, length = 100)
        LLs =
            map(λ_s_k -> sumlogpdf(MarkovModel(; parms..., λ_s_k), n_options, data), λ_s_ks)
        _, idx = findmax(LLs)
        @test λ_s_ks[idx] ≈ λ_s_k rtol = 0.10

        λ_k_ss = range(λ_k_s * 0.8, λ_k_s * 1.2, length = 100)
        LLs =
            map(λ_k_s -> sumlogpdf(MarkovModel(; parms..., λ_k_s), n_options, data), λ_k_ss)
        _, idx = findmax(LLs)
        @test λ_k_ss[idx] ≈ λ_k_s rtol = 0.10

        λ_s_ss = range(λ_s_s * 0.8, λ_s_s * 1.2, length = 100)
        LLs =
            map(λ_s_s -> sumlogpdf(MarkovModel(; parms..., λ_s_s), n_options, data), λ_s_ss)
        _, idx = findmax(LLs)
        @test λ_s_ss[idx] ≈ λ_s_s rtol = 0.10
    end
    ...
    @safetestset "Quantum" begin
        ...
        υ_k_ks = range(υ_k_k * 0.8, υ_k_k * 1.2, length = 100)
        LLs = map(
            υ_k_k -> sumlogpdf(QuantumModel(; parms..., υ_k_k), n_options, data),
            υ_k_ks
        )
        _, idx = findmax(LLs)
        @test υ_k_ks[idx] ≈ υ_k_k rtol = 0.10
        ...
        υ_s_ks = range(υ_s_k * 0.8, υ_s_k * 1.2, length = 100)
        LLs = map(
            υ_s_k -> sumlogpdf(QuantumModel(; parms..., υ_s_k), n_options, data),
            υ_s_ks
        )
        _, idx = findmax(LLs)
        @test υ_s_ks[idx] ≈ υ_s_k rtol = 0.10
        ...
        υ_k_ss = range(υ_k_s * 0.8, υ_k_s * 1.2, length = 100)
        LLs = map(
            υ_k_s -> sumlogpdf(QuantumModel(; parms..., υ_k_s), n_options, data),
            υ_k_ss
        )
        _, idx = findmax(LLs)
        @test υ_k_ss[idx] ≈ υ_k_s rtol = 0.10
        ...
        υ_s_ss = range(υ_s_s * 0.8, υ_s_s * 1.2, length = 100)
        LLs = map(
            υ_s_s -> sumlogpdf(QuantumModel(; parms..., υ_s_s), n_options, data),
            υ_s_ss
        )
        _, idx = findmax(LLs)
        @test υ_s_ss[idx] ≈ υ_s_s rtol = 0.10
        ...
        λ_k_ks = range(λ_k_k * 0.8, λ_k_k * 1.2, length = 100)
        LLs = map(
            λ_k_k -> sumlogpdf(QuantumModel(; parms..., λ_k_k), n_options, data),
            λ_k_ks
        )
        _, idx = findmax(LLs)
        @test λ_k_ks[idx] ≈ λ_k_k rtol = 0.10
        ...
        λ_s_ks = range(λ_s_k * 0.8, λ_s_k * 1.2, length = 100)
        LLs = map(
            λ_s_k -> sumlogpdf(QuantumModel(; parms..., λ_s_k), n_options, data),
            λ_s_ks
        )
        _, idx = findmax(LLs)
        @test λ_s_ks[idx] ≈ λ_s_k rtol = 0.10
        ...
        λ_k_ss = range(λ_k_s * 0.8, λ_k_s * 1.2, length = 100)
        LLs = map(
            λ_k_s -> sumlogpdf(QuantumModel(; parms..., λ_k_s), n_options, data),
            λ_k_ss
        )
        _, idx = findmax(LLs)
        @test λ_k_ss[idx] ≈ λ_k_s rtol = 0.10
        ...
        λ_s_ss = range(λ_s_s * 0.8, λ_s_s * 1.2, length = 100)
        LLs = map(
            λ_s_s -> sumlogpdf(QuantumModel(; parms..., λ_s_s), n_options, data),
            λ_s_ss
        )
        _, idx = findmax(LLs)
        @test λ_s_ss[idx] ≈ λ_s_s rtol = 0.10
